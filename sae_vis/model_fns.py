...
class HookedTransformerWrapper(nn.Module):
    ...
    def forward(self, tokens: Int[Tensor, "batch seq"]) -> tuple[Tensor, Tensor, Tensor]:
        """
        Inputs:
        ...
        def hook_fn_store_act(activation: torch.Tensor, hook: HookPoint):
            act = activation.detach().clone()
            assert isinstance(hook.name, str)

            match hook.name.split(".hook_")[-1]:
                case "resid_pre" | "resid_mid" | "resid_post" | "attn_out" | "mlp_out":
                    assert act.shape == (*tokens.shape, self.cfg.d_model)
                case "pre" | "post":
                    assert act.shape == (*tokens.shape, self.cfg.d_mlp)
                case "z":
                    assert act.shape == (
                        *tokens.shape,
                        self.cfg.n_heads,
                    ...
                    act = einops.rearrange(act, "batch seq n_heads d_head -> batch seq (n_heads d_head)")
                case _:
                    raise ValueError(
                        f"Unexpected hook name: {hook.name}, split to {hook.name.split('.hook_')[-1]} which had no match"
                    )
        ...
        logits: Tensor = self.model.run_with_hooks(
            tokens,
            fwd_hooks=[
                (self.hook_point, hook_fn_store_act),
                (self.hook_point_resid_final, hook_fn_store_act),
            ],
        ...
        activation = self.model.hook_dict[self.hook_point].ctx.pop("act")
        if self.hook_point_resid_final == self.hook_point:
            residual = activation
        else:
            residual = self.model.hook_dict[self.hook_point_resid_final].ctx.pop("act")
...
def to_resid_dir(dir: Float[Tensor, "feats d"], model: HookedTransformerWrapper, input: bool = False):
    """
    ...
    match model.hook_point.split(".hook_")[-1]:
        case "resid_pre" | "resid_mid" | "resid_post" | "attn_out" | "mlp_out":
            return dir
        case "pre" | "post":
            return dir @ (model.W_in[model.hook_layer].T if input else model.W_out[model.hook_layer])
        case "z":
            return dir @ (
                einops.rearrange(
                    model.W_V[model.hook_layer],
                    "n_heads d_model d_head -> (n_heads d_head) d_model",
                )
                if input
                else einops.rearrange(
                    model.W_O[model.hook_layer],
...
def load_othello_data_as_in_colab(n_games: int, remove_last_move: bool = True) -> Tensor:
...
def load_othello_sae(dir_name: str) -> SAE:
    ...
    """
    OTHELLO_SAE_PATH = "C:/Users/calsm/Documents/AI Alignment/sae-vis/trained_saes/othello"
    ...
    with safe_open(f"{OTHELLO_SAE_PATH}/{dir_name}/sae_weights.safetensors", framework="pt") as f:  # type: ignore
...
def load_paper_othello_models(
    ...
    othello_paper_model = othello_models / f"{filename.removesuffix('.zip')}/{model_name}"
    ...
    ae_weights_renamed = {rename_dict[k]: (v if v.ndim == 1 else v.T).contiguous() for k, v in ae_weights.items()}
...
def load_demo_othello_model_saes_and_data(device: str):
    ...
    """
    model = HookedSAETransformer.from_pretrained("othello-gpt", center_writing_weights=False).to(device).eval()
    ...
    target_logits: Tensor = torch.load(local_dir + "/target_logits.pt").float().to(device)
